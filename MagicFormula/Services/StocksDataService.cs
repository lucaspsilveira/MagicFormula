...
public class StocksDataService
{
    ...
    private const string _statusInvestAddress = "https://statusinvest.com.br";

    public StocksDataService()
    {
        _httpClient = new HttpClient
        {
            BaseAddress = new Uri(_statusInvestAddress)
        };
    ...
    {
        var requestUri = "category/advancedsearchresult";
        var filterQuery =
        ...
        var result = await _httpClient.GetStringAsync(requestUri + filterQuery);
        return JsonSerializer.Deserialize<List<Stock>>(result) ?? new List<Stock>();
    }
}